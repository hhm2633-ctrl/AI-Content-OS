...
- `docs/CARDNEWS_MULTI_ACCOUNT_DIRECTIVE.md`가 최우선이다. 4장
  `hook`/`problem`/`solution`/`cta`는 레거시 템플릿 중 하나일 뿐 고정 계약이 아니다.
  계정·카테고리·주제별 최신 Instagram CardNews/carousel 관찰 근거로 슬라이드 수와 role을
  가변 결정한다. 현재 4장 전제 코드가 있다면 제품 요구사항으로 보호하지 말고 명시적인
  가변 길이 마이그레이션 대상으로 보고한다.
