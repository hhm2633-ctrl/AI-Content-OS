...
이 스킬은 AI-Content-OS에서 Codex가 어떤 작업(Sprint, 디버깅, 리서치 반영, 단발성 요청)을
시작하든 가장 먼저 확인하는 진입점이다. 다른 모든 `.Codex/skills/*` 파일보다 먼저 읽는다.
...
1. **`PROJECT_OPERATING_SYSTEM.md`를 읽는다.** 이 문서의 "Mandatory Reading Order"를 그대로
   ...
   9. `.Codex/skills/*` (Codex가 명시적으로 배정된 경우의 호환 지침)
...
5. 항상 Repository 성장과 ROI를 최대화하는 방향으로 작업 범위를 판단한다
   (`.Codex/skills/planning.md`의 Sprint ROI 평가와 함께 사용).

## 이 스킬이 다른 스킬과 관계

- `.Codex/skills/architecture.md`가 프로젝트 상태를 읽는 구체적인 절차(WorkflowEngine 구조,
...
- Protected Core(`WorkflowEngine`의 10개 핵심 모듈)와 Intelligence Layer(Knowledge/Trend
  ...
  `.Codex/skills/large_implementation.md`의 Engine Rule과 동일한 내용이다.
- 새로운 Research 자료 반영은 `.Codex/skills/research.md`를, Sprint 범위 판단은
  `.Codex/skills/planning.md`를, 완료 후 점검은 `.Codex/skills/review.md`를 따른다 —
...
- WorkflowEngine의 실제 모듈 순서/구조 세부사항은 `.Codex/skills/architecture.md`를 따른다.
