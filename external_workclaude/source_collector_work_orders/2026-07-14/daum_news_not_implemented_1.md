...
## objective
- implement the `daum_news` collector path and wire source intake metadata for status `NOT_IMPLEMENTED`

## owned files
- modules/trend_collector/daum_news_collector.py when this source needs a new collector
- modules/source_intake/source_capability_map.py only if capability metadata must be extended
- tests/test_daum_news_collector.py or focused source-intake collector tests
...
- affected lanes: entertainment_news, lifestyle_knowledge, news_society_economy
