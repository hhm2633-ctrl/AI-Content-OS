...
| `tests.test_naver_news_parser_recovery` | 2건 실패 잔존 |
| 실제 `collect(["AI"], source)` 관찰 | 5건 반환, API Hub 경로 사용 |
...
3. 네이버 뉴스 작업을 계속하라는 요청이 오면 위 recovery 실패 2건만 수정한다.
4. 관련 테스트 전체와 실제 네이버 검색 1회를 마지막에 실행한다.
