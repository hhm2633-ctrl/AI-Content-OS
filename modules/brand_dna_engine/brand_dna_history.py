...
class BrandDNAHistory(object):
    """
    Brand DNA Engine - History.

    storage/brand_dna/brand_dna_history.json에 실행별 관찰(observation)을
    append-only로 기록한다.
    """
    ...
    def __init__(self, history_path: Optional[Path] = None):
        self.history_path = history_path or Path("storage/brand_dna/brand_dna_history.json")
        self.history_path.parent.mkdir(parents=True, exist_ok=True)

    def record(self, observation: Dict[str, Any]) -> None:
        try:
            self._record(observation or {})
        except Exception as error:
            print(f"Brand DNA History Write Failed: {error}")
    ...
    def _record(self, observation: Dict[str, Any]) -> None:
        ...
        records.append({"recorded_at": datetime.now().isoformat(), **observation})
