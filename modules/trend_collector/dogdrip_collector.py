...
class DogdripCollector:
    """Fixture/cache-ready Dogdrip list collector with live access disabled."""

    BASE_URL = "https://www.dogdrip.net"
    DEFAULT_CACHE_PATH = Path("storage/cache/dogdrip_cache.json")
    ...
    def collect(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if rows:
            method = "dogdrip_live_html" if live_enabled else "dogdrip_fixture_html"
            items = self._dedupe(self._build_items(rows, source))[: self.max_items]
            if items:
                for item in items:
                    item["collection_method"] = method
                    item["trend_reason"] = f"Dogdrip {'live' if live_enabled else 'fixture'} popular-list collection"
            ...
            self.last_status.update(
                {
                    ...
                    "collection_method": "dogdrip_cache",
                    "used_cache": True,
        ...
        self.last_status.update(
            {
                ...
                "fallback_reason": reason,
                "final_error_type": reason,
                "collection_method": "dogdrip_no_data",
                "service_diagnostic": self._diagnostic(reason, "fallback_used"),
            }
        )
        return []
    ...
    def _parse_fixture_html(self, raw_html: str) -> List[Dict[str, Any]]:
            ...
            rows.append(
                {
                    ...
                    or self._cell_text(row_html, ("category", "cate", "board")),
                    "views": self._metric(
                        open_tag, row_html, "data-views", ("readnum", "views", "hit")
                    ),
                    "comments": self._metric(
                        open_tag,
                        row_html,
                        "data-comments",
                        ("replynum", "comments", "comment", "reply"),
                    ),
                    "likes": self._metric(
                        open_tag,
                        row_html,
                        "data-likes",
                        ("votenum", "voted", "recommend", "likes"),
                    ),
        ...
        if not rows:
            rows = self._parse_popular_cards(raw_html)

        return rows[: self.max_items]

    def _parse_popular_cards(self, raw_html: str) -> List[Dict[str, Any]]:
        rows: List[Dict[str, Any]] = []
        for card_html in re.findall(
            r"<li\b[^>]*class=[\"'][^\"']*\bpopular-item\b[^\"']*[\"'][^>]*>.*?</li>",
            raw_html,
            flags=re.IGNORECASE | re.DOTALL,
        ):
            anchor = re.search(
                r"<a\b([^>]*)class=[\"'][^\"']*\btitle-link\b[^\"']*[\"'][^>]*>(.*?)</a>",
                card_html,
                flags=re.IGNORECASE | re.DOTALL,
            )
            if not anchor:
                continue
            attrs, content = anchor.groups()
            href = self._attr(f"<a {attrs}>", "href")
            title = self._clean_text(content)
            if not title or not re.search(r"/dogdrip/\d+", href):
                continue

            tail = card_html[anchor.end():]
            comment_match = re.search(
                r"<span[^>]*class=[\"'][^\"']*\btext-primary\b[^\"']*\btext-xxsmall\b[^\"']*[\"'][^>]*>(.*?)</span>",
                tail,
                flags=re.IGNORECASE | re.DOTALL,
            )
            likes_match = re.search(
                r"fa-thumbs-up.*?</span>\s*<span[^>]*>(.*?)</span>",
                card_html,
                flags=re.IGNORECASE | re.DOTALL,
            )
            ...
            rows.append(
                {
                    "title": title,
                    "link": self._normalize_link(href),
                    "published_at": self._clean_text(time_match.group(1)) if time_match else "",
                    "category": "인기",
                    "views": None,
                    "comments": self._int_or_none(comment_match.group(1)) if comment_match else None,
                    "likes": self._int_or_none(likes_match.group(1)) if likes_match else None,
                    "rank_position": len(rows) + 1,
                }
            )
        return rows
            ...
            items.append(
                {
                    ...
                    "source_id": "dogdrip",
                    "source_name": source.get("name", "Dogdrip"),
                    ...
                    "trend_reason": "Dogdrip fixture list collection",
                    "collection_method": "dogdrip_fixture_html",
                    "is_fallback": False,
            ...
            item.update(
                {
                    ...
                    "weight": int(source.get("weight", 0) or 0),
                    "base_score": max(95 - rank, 1),
                    "trend_reason": "Dogdrip cache fallback",
                    "collection_method": "dogdrip_cache",
                    "is_fallback": True,
    ...
    def _title_anchor(self, row_html: str) -> Optional[Tuple[str, str]]:
        ...
        for attrs, content in anchors:
            ...
            if (
                "title" in css_class
                or "link-reset" in css_class
                or "document_srl" in href
                or "/dogdrip" in href
            ):
                return href, self._clean_text(content)
        return None
    ...
    def _diagnostic(self, reason: str, status: str) -> Dict[str, Any]:
        return self.service_diagnostic.build_diagnostic_from_reason(
            service="dogdrip", reason=reason, status=status
        )
