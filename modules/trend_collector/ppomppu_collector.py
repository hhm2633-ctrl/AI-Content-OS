...
class PpomppuCollector:
    """Fixture/cache-ready PPOMPPU list collector with live access disabled."""

    BASE_URL = "https://www.ppomppu.co.kr"
    DEFAULT_CACHE_PATH = Path("storage/cache/ppomppu_cache.json")
    ...
    def collect(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if rows:
            method = "ppomppu_live_html" if live_enabled else "ppomppu_fixture_html"
            items = self._dedupe(self._build_items(rows, source))[: self.max_items]
            if items:
                for item in items:
                    item["collection_method"] = method
                    item["trend_reason"] = f"PPOMPPU {'live' if live_enabled else 'fixture'} list collection"
            ...
            self.last_status.update(
                {
                    ...
                    "collection_method": "ppomppu_cache",
                    "used_cache": True,
        ...
        self.last_status.update(
            {
                ...
                "fallback_reason": reason,
                "final_error_type": reason,
                "collection_method": "ppomppu_no_data",
                "service_diagnostic": self._diagnostic(reason, "fallback_used"),
            }
        )
        return []
    ...
    def _parse_fixture_html(self, raw_html: str) -> List[Dict[str, Any]]:
        ...
        for row_html in re.findall(
            ...
            board_dates = re.findall(
                r"<td[^>]*class=[\"'][^\"']*\bboard_date\b[^\"']*[\"'][^>]*>(.*?)</td>",
                row_html,
                flags=re.IGNORECASE | re.DOTALL,
            )
            board_values = [self._clean_text(value) for value in board_dates]
            vote_parts = re.findall(r"\d+", board_values[1]) if len(board_values) > 1 else []
            comment_match = re.search(
                r"<span[^>]*class=[\"'][^\"']*\blist_comment2\b[^\"']*[\"'][^>]*>(.*?)</span>",
                row_html,
                flags=re.IGNORECASE | re.DOTALL,
            )
            views = self._metric(open_tag, row_html, "data-views", ("views", "list_hit"))
            comments = self._metric(
                open_tag, row_html, "data-comments", ("comments", "comment", "list_comment", "reply")
            )
            likes = self._metric(open_tag, row_html, "data-likes", ("likes", "recommend", "list_vote"))
            dislikes = self._metric(open_tag, row_html, "data-dislikes", ("dislikes", "list_dislike"))
            ...
            rows.append(
                {
                    ...
                    or (board_values[0] if board_values else "")
                    or self._cell_text(row_html, ("date", "list_date", "time")),
                    "category": self._attr(open_tag, "data-category")
                    or self._cell_text(row_html, ("baseList-numb", "category", "list_category")),
                    "views": views if views is not None else (self._int_or_none(board_values[2]) if len(board_values) > 2 else None),
                    "comments": comments if comments is not None else (self._int_or_none(comment_match.group(1)) if comment_match else None),
                    "likes": likes if likes is not None else (self._int_or_none(vote_parts[0]) if vote_parts else None),
                    "dislikes": dislikes if dislikes is not None else (self._int_or_none(vote_parts[1]) if len(vote_parts) > 1 else None),
            ...
            items.append(
                {
                    ...
                    "dislikes": self._int_or_none(row.get("dislikes")),
                    "source_id": "ppomppu",
                    "source_name": source.get("name", "PPOMPPU"),
                    ...
                    "trend_reason": "PPOMPPU fixture list collection",
                    "collection_method": "ppomppu_fixture_html",
                    "is_fallback": False,
            ...
            item.update(
                {
                    ...
                    "dislikes": self._int_or_none(item.get("dislikes")),
                    "source_id": "ppomppu",
                    "source_name": source.get("name", "PPOMPPU"),
                    "source_type": source.get("type", "community"),
                    "tier": int(source.get("tier", 50) or 50),
                    "weight": int(source.get("weight", 0) or 0),
                    "base_score": max(95 - rank, 1),
                    "trend_reason": "PPOMPPU cache fallback",
                    "collection_method": "ppomppu_cache",
                    "is_fallback": True,
    ...
    def _title_anchor(self, row_html: str) -> Optional[Tuple[str, str]]:
        ...
        for attrs, content in anchors:
            ...
            if "title" in css_class or "/zboard/view.php" in href:
    ...
    def _diagnostic(self, reason: str, status: str) -> Dict[str, Any]:
        return self.service_diagnostic.build_diagnostic_from_reason(
            service="ppomppu", reason=reason, status=status
        )
